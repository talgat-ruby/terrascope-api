"""terrascope2 CLI — per-process pipeline (experimental).
...
app = typer.Typer(
    name="terrascope2", help="Terrascope cli2 — per-process pipeline (experimental)"
)
...
def run(
    ...
    if list_processes:
        from core2 import registered_names
...
def _run(job: JobConfig) -> None:
    ...
    from core2 import ProcessSpec, build, render_overlay, run_processes
